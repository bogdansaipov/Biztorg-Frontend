      ...
      <div className="max-w-[1400px] mx-auto px-4 py-6">
        <div className="flex flex-col gap-2 lg:gap-3 lg:max-w-[933px]">
          {products === null ? (
            ...
            <p className="text-gray-400 text-sm text-center py-10">У этого магазина пока нет объявлений.</p>
          ) : (
            products.map((product) => {
              const mainImage = product.images.find((i) => i.isMain)?.imageUrl ?? product.images[0]?.imageUrl;
              // isRatedByCurrentUser comes straight from GET /shop-profiles/{id}/products —
              // once true, a rating already exists for this product from
              // the current viewer, so it can't be rated again.
              ...
              return alreadyRated ? (
                ...
                <Link key={product.id} href={`/shop/${shopId}/rate/${product.id}`}>
                  {content}
                </Link>
              );
            })

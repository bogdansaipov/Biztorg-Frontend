...
import { ArrowLeft, X, ImageSquareIcon } from "@phosphor-icons/react";
...
export default function RateShopProductPage() {
  const params = useParams<{ id: string; productId: string }>();
  const router = useRouter();
  const pathname = usePathname();
  const { id: shopId, productId } = params;

  // /shop/... is a non-region route (same as ShopRatingsPage), so this
  // is all that's needed. Previously missing entirely here — the
  // post-submit redirect below was going to a bare "/shop/{id}" with no
  // locale segment at all.
  const locale = pathname.split("/")[1] || "ru";
  const t = useTranslations("rateProduct");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/${locale}/shop/${shopId}`);
    } catch (err) {
    ...
    <div className="min-h-screen w-full bg-gray-50">
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-[1400px] mx-auto flex items-center justify-between px-4 lg:px-6 py-3">
          ...
          <button onClick={() => router.back()} className="cursor-pointer p-1 -mr-1" aria-label={t("close")}>
      ...
      <div className="max-w-[1400px] mx-auto px-4 lg:px-6 py-6">
        <div className="max-w-[560px] lg:max-w-[640px] mx-auto space-y-4 lg:space-y-5">
          <div className="bg-white border border-gray-100 rounded-2xl p-6 lg:p-8 flex flex-col items-center text-center">
              ...
              {Array.from({ length: 5 }).map((_, i) => {
                ...
                return (
                  <button
                    ...
                    aria-label={t("starAria", { star: starValue })}
                  >
                    <StarIcon
                      weight="fill"
            ...
            <p className="text-sm text-gray-500 mt-3">{RATING_LABELS[displayValue] ?? t("ratingPrompt")}</p>
          </div>

          <div className="bg-white border border-gray-100 rounded-2xl p-5 lg:p-6">
            <h2 className="font-semibold text-gray-900 mb-1">{t("shareTitle")}</h2>
            <p className="text-sm text-gray-500 mb-3">{t("shareSubtitle")}</p>
            <textarea
              value={comment}
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5 lg:p-6">
            <h2 className="font-semibold text-gray-900 mb-1">{t("photosTitle")}</h2>
            <p className="text-sm text-gray-500 mb-3">
              {t("photosSubtitle", { max: MAX_IMAGES })}
            </p>
